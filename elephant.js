function drawPixelElephant(ctx, x, y, scale, dir, state, frame, animTime, species = 'african', isWet = false) {
      ...
      if (y > 124) {
        ctx.save();
        ctx.scale(dir * scale, -scale * 0.35);
        ctx.translate(0, -10);
        ctx.globalAlpha = 0.2;
      ...
      const legPhase = isWalking ? (animTime * 0.12) % (Math.PI * 2) : 0;
      const leg1 = isWalking ? Math.sin(legPhase) * 2.5 : 0;
      const trunkSway = Math.sin(animTime * 0.1) * 3;

      if (isSubmerged) {
        // --- A. UNDERWATER SUBMERGED BODY (SEMI-TRANSPARENT WITH RIVER BLUE TINT) ---
        ctx.save();
        ctx.globalAlpha = 0.38; // Semi-transparent submerged dog-paddling body!

        ctx.fillStyle = cDark;
        // Far Back Leg
        ctx.fillRect(-13, -6, 6, 6); ctx.fillRect(-13 + leg1, -2, 5, 11);
        // Far Front Leg
        ctx.fillRect(7, -6, 6, 6); ctx.fillRect(7 - leg1, -2, 5, 11);
        ...
        // --- B. HEAD, EARS, TUSK & TRUNK EMERGING ABOVE WATER (FULL OPACITY) ---
        ctx.save();
        ctx.globalAlpha = 1.0;
        ...
        let earFlap = 1.0;
        if (state !== 'flee') {
          earFlap = 0.6 + Math.sin(animTime * 0.12) * 0.4;
        }
        ctx.save();
        ctx.translate(5, -18);
        ctx.scale(earFlap, 1.0);
        ...
        // Trunk (Raised high or snorkel pose when bathing/swimming)
        ctx.fillStyle = cMain;
        if (state === 'bath') {
          // Curved up spraying fountain water
          ctx.fillRect(20, -22, 6, 6);
          ctx.fillRect(17, -28, 5, 8);
          ctx.fillRect(12, -35, 5, 8);
        } else {
        ...
        // Water surface foam line around shoulders
        ctx.fillStyle = 'rgba(255, 255, 255, 0.88)';
        ctx.fillRect(-22, -14 + Math.sin(animTime * 0.2) * 0.8, 44, 2);
        ctx.fillStyle = 'rgba(255, 255, 255, 0.45)';
        ctx.fillRect(-24, -13 + Math.sin(animTime * 0.2) * 0.8, 48, 1);
        ...
        // --- STANDARD ONSHORE DRY-LAND ELEPHANT RENDER ---
        ctx.fillStyle = cDark;
        // Far Back Leg
    ...
    // DRAW ZOOKEEPER CLEANER WITH ONE-WHEELED WHEELBARROW & SHOVEL