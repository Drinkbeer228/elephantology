...
const regex = /(const res = await fetch\(`\/api\/article\?path=\$\{encodeURIComponent\(safePath\)\}`\);[\s\S]*?)(const html = await marked\.parse\(md\);[\s\S]*?if \(contentEl\) contentEl\.innerHTML = html;)/;

const match = code.match(regex);
if (!match) {
  console.log("Could not match the target block.");
  process.exit(1);
}

const replacement = `    const res = await fetch(\`/api/article?path=\${encodeURIComponent(safePath)}\`);
...
// We need to inject the showFootnoteModal function right before loadArticle definition.
const footnoteModalHtml = `
...
code = code.replace('(window as any).loadArticle = async (path: string) => {', footnoteModalHtml);

fs.writeFileSync('src/main.tsx', code);
console.log("Done patching main.tsx");
