...
export function OfflineModal() {
    ...
    const update = () => {
      ...
      if (savedSync) {
        try {
          ...
          setLastSync(date.toLocaleDateString('ru-RU', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }));
        } catch(e) {}
      }
    };
  ...
  const handleClear = async () => {
    if (window.confirm('Очистить сохраненные статьи из оффлайн-памяти?')) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                {isOnline ? '🟢 Подключение к сети активно' : '🟠 Автономный режим (Оффлайн)'}
              </p>
            </div>
          </div>
          <button 
          ...
          {isInstallable && (
            <div className="p-4 rounded-xl bg-gradient-to-r from-kingdom-gold/10 to-amber-500/5 border border-kingdom-gold/30 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-white text-xs sm:text-sm">Установить как приложение</h4>
                  <p className="text-[11px] text-gray-300">Быстрый запуск с рабочего стола и полноэкранный режим</p>
                </div>
              </div>
              <button
          ...
          <div className="p-5 rounded-xl bg-[#121318]/70 border border-white/5 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-bold text-sm text-white">Оффлайн-хранилище статей</span>
              </div>
              <span className="text-xs font-mono px-2.5 py-0.5 rounded-full bg-kingdom-gold/10 text-kingdom-gold border border-kingdom-gold/20">
                {cachedCount > 0 ? `${cachedCount} статей в кэше` : 'Кэш пуст'}
              </span>
            </div>
            ...
            {isSyncing ? (
              <div className="space-y-2 pt-2">
                <div className="flex justify-between text-xs text-gray-300">
                  <span className="flex items-center gap-1.5 text-kingdom-gold">
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Загрузка в память...
                  </span>
                ...
                {syncProgress.currentTitle && (
                  <p className="text-[10px] text-gray-400 truncate">
                    Сохранение: {syncProgress.currentTitle}
                  </p>
                )}
              </div>
            ) : (
              <div className="pt-2 flex flex-col sm:flex-row gap-2.5">
                ...
                >
                  ...
                  <span>{cachedCount > 0 ? 'Обновить всю оффлайн-базу' : 'Скачать всю энциклопедию (50 статей)'}</span>
                </button>
                {cachedCount > 0 && (
                  <button
                    onClick={handleClear}
                    title="Очистить сохраненные статьи"
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex items-start gap-2.5">
                ...
                <div>
                  <span className="font-semibold text-white block">50 научных статей</span>
                  <span className="text-[10px] text-gray-400">Анатомия, этология, эволюция</span>
                </div>
              </div>
              <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex items-start gap-2.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                <div>
                  <span className="font-semibold text-white block">Схемы и таблицы</span>
                  <span className="text-[10px] text-gray-400">Интерактивный атлас и оглавления</span>
                </div>
              </div>
              <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex items-start gap-2.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                <div>
                  <span className="font-semibold text-white block">Карты и карточки</span>
                  <span className="text-[10px] text-gray-400">Флешкарты и ареалы обитания</span>
                </div>
              </div>
              <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex items-start gap-2.5">
                <Sparkles className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                <div>
                  <span className="font-semibold text-amber-200 block">AI-Слонолог</span>
                  <span className="text-[10px] text-gray-400">Требует онлайн-подключение</span>
                </div>
              </div>
            </div>
          </div>
